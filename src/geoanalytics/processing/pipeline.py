...
def _process_news(session: Session, doc: RawDocument, index: EntityIndex,
    ...
    if settings.dedup_window_hours and gp._is_duplicate(
        session, chash, settings.dedup_window_hours
    ):
        result.duplicates += 1
        return True
    ...
    significance = gp._compute_significance(
    ...
    is_fc = forecast.is_forecast_post(
        title, body, channel=channel, temporal_status=t_status
    )
    ...
    article = Article(
        ...
        source_ref=channel[:64] if channel else None,
