...
from geoanalytics.nlp._seqcls import ModelConfig, ModelLoader, SeqClsAdapter
...
_RULES: list[tuple[EventType, re.Pattern]] = [
    (
        EventType.SANCTIONS,
        re.compile(
            r"санкц|эмбарго|ограничени.{0,15}поставок|"
            r"чёрн.{0,3}список",
            re.I,
        ),
    ),
    (
        EventType.DIVIDENDS,
        re.compile(
            r"дивиденд|выплат.{0,10}акционер|отсечк",
            re.I,
        ),
    ),
    (
        EventType.MERGER,
        re.compile(
            r"слияни|поглощени|приобрел.{0,5}долю|"
            r"сделк.{0,5}M&A|выкуп.{0,10}акц",
            re.I,
        ),
    ),
    (
        EventType.EARNINGS,
        re.compile(
            r"отчётност|отчетност|выручк|чист.{0,5}прибыл|"
            r"чист.{0,5}убыт|EBITDA|финрезультат",
            re.I,
        ),
    ),
    (
        ...
        re.compile(
            r"регулятор|законопроект|постановлени|"
            r"ЦБ.{0,15}требовани|лицензи|налог",
            re.I,
        ),
    ),
    (
        EventType.MACRO,
        re.compile(
            r"ключев.{0,5}ставк|инфляц|ВВП|курс.{0,5}рубл|"
            r"нефт|бюджет|ставк.{0,5}ЦБ",
            re.I,
        ),
    ),
    (
        ...
        re.compile(
            r"переговор|саммит|конфликт|дипломат|"
            r"соглашени.{0,15}стран|внешн.{0,5}политик",
            re.I,
        ),
    ),
]
